...
<table>
	<c:forEach items="${commentsListQa}" var="comments">
		<tr>
			...
			<td width="60"><c:if
					test="${USER_KEY.userEmail == comments.userEmail }">
					<input type="hidden" name="bdNoQaComments"
						value="${comments.bdNoQaComments}" />
					<img id="checkPass"
						src="<%=request.getContextPath() %>/images/icon/x.png"
						onclick="really(${comments.bdNoQaComments})" class="pointer"
						width="30" height="30">
				</c:if></td>
		</tr>
		<tr height="1">
	...
	<c:if test="${!empty USER_KEY}">
		<table width="960" border="0">
			<tr>
				<td>
					<form method="post"
						action='boardQaWriCom.html?pageNo=${pageNo }&bdNoQa=${boardQa.bdNoQa}'>
						<input type="hidden" name="bdNoQa" value="${boardQa.bdNoQa }" />
						<input type="hidden" name="pageNo" value="${pageNo }" />
						<textarea rows="3" cols="100" name="bdQaCommentsContent"
