		...
		<c:if test="${!empty articleListRev}">
			<table>
				...
				<c:forEach items="${articleListRev}" var="reviews">
					<tr class="record">
						<td align="left" class="listtxt"><c:out
								value="${reviews.bdNoRev}" /></td>
						<td align="left" class="listtxt"><c:choose>
								<c:when test="${reviews.reStep == 0}">
									<a
										href="<c:url value="boardReviewsDetail.html">
						...
						<td align="center" class="listtxt"><c:out
								value="${reviews.userAlias}" /></td>
						<td align="center" class="listtxt"><font size="0.1"><c:out
									value="${reviews.dateRev}" /></font></td>
						<td align="center" class="listtxt"><c:out
								value="${reviews.countRev}" />/ <c:out
								value="${reviews.recommendRev}" /></td>
					</tr>
				</c:forEach>
			</table>
			<!-- 하단부 페이지 이동버튼 만들기 -->
